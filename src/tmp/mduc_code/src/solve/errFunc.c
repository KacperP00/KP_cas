...
static int ncof = 28;
static double cof[28] = {-1.3026537197817094,      6.4196979235649026e-1,
...
double inverfc(double p)
{
  int j;
  double x, err, t, pp;
  if (p >= 2.0) return -100.0; /* return arbitrary large pos/neg value */
  if (p <= 0.0) return 100.0;
  pp = (p < 1.0) ? p : 2.0 - p;
  t = sqrt(-2.0*log(0.5*pp));    /* initial guess */
...
double erfccheb(double z)
{
  ...
  if (z < 0.0)
    {
      fprintf(stderr, "Error: erfccheb requires non-negative argument\n");
      exit(2);
    }
